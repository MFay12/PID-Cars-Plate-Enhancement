"""Heuristic license plate detector based on classical computer vision.

Candidates are exposed in the format used by this repository:
...
class HeuristicPlateDetector:
    """Locates and ranks plates using classical vision and a compound score."""
    ...
    def _character_features(self, roi, family_name):
        ...
        def analyze_mask(mask):
            ...
            if components:
                ...
                center_ys = np.asarray(
                    [c[1] + c[3] / 2.0 for c in components], dtype=np.float32
                )
                consistency = self._clip01(
                    1.0 - float(np.std(heights)) / (float(np.mean(heights)) + 1e-6)
                )
                if family_name == "horizontal":
                    consistency *= self._clip01(
                        1.0 - float(np.std(center_ys)) / (height * 0.35 + 1e-6)
                    )
                coverage = sum(c[4] for c in components) / roi_area
        ...
        if self.adaptive_threshold_fallback and best["score"] < 0.45:
            block_size = min(31, max(3, (min(height, width) // 2) * 2 - 1))
            if block_size >= 3:
                adaptive = cv2.adaptiveThreshold(
                    roi, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, block_size, 5
                )
    ...
    def _candidate_score(self, enhanced, box, family, contour_fill):
        ...
        log_area_error = abs(math.log(max(area_ratio, 1e-8) / family["area_target"]))
        ...
        position_score = 0.5 * self._gaussian_score(center_x, 0.45, 0.30) + 0.5 * self._gaussian_score(center_y, 0.72, 0.22)
    ...
    def search(self, image, families=("car", "motorcycle"), limit=5):
        """Returns candidates ``(score, x, y, width, height)`` ordered by score."""
        ...
        for family_key in families:
            if family_key not in PLATE_FAMILIES:
                raise ValueError(f"Unknown plate family: {family_key!r}")
            selected_families.append(PLATE_FAMILIES[family_key])
        ...
        light_regions = cv2.morphologyEx(enhanced, cv2.MORPH_CLOSE, light_kernel)
        ...
        for family in selected_families:
            for (
                ...
                if mask is None:
                    ...
                    mask = self._directional_character_mask(enhanced, light_regions, kernel)
                    directional_masks[mask_key] = mask
                if group_kernel_640:
                    mask = cv2.morphologyEx(
                        mask, cv2.MORPH_CLOSE, self._scaled_kernel(*group_kernel_640)
                    )
                ...
                for contour in contours:
                    ...
                    for padding_x, padding_y in expansions:
                        ...
                        right = min(self.normalized_width, x + width + round(width * padding_x))
                        bottom = min(normalized_height, y + height + round(height * padding_y))
                        ...
                        minimum_area = max(self.min_area_ratio, family["area_min"])
                        maximum_area = min(self.max_area_ratio, family["area_max"])
                        if not (
                            minimum_area <= area_ratio <= maximum_area
                            and family["ratio_min"] <= ratio <= family["ratio_max"]
                        ):
                            continue
                        preliminary_score = (
                            0.40 * self._gaussian_score(ratio, family["ratio_target"], family["ratio_sigma"])
                            + 0.45 * self._gaussian_score(abs(math.log(max(area_ratio, 1e-8) / family["area_target"])), 0.0, 1.15)
        ...
        raw_candidates.sort(key=lambda item: item["preliminary_score"], reverse=True)
        unique_candidates = []
        ...
        accepted = [c for c in candidates if c["score"] >= self.score_threshold]
        ...
        output = []
        for candidate in accepted[: max(0, int(limit))]:
            x1, y1, x2, y2 = candidate["box"]
            output.append((float(candidate["score"]), int(x1), int(y1), int(x2 - x1), int(y2 - y1)))
        return output

def draw_candidates(color_img, candidates, color):
    """Generates visual debug in the same format as the original PID functions."""
    if color_img is None:
        return None
    debug_img = color_img.copy()
    for index, candidate in enumerate(candidates):
        score, x, y, width, height = candidate
        cv2.rectangle(debug_img, (x, y), (x + width, y + height), color, 1)
        text_y = max(12, y - 5)
        cv2.putText(
            debug_img, f"Test #{index + 1} ({score:.2f})", (x, text_y),
